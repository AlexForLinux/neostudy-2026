# Summary

# Оценка LLMs (10.03 - 24.03)
1. [Декомпозиция AI-системы для оценки](./evaluateLLMs/decompose.md)
2. [Критерии оценки AI-систем](./evaluateLLMs/criterion.md)
3. [Подходы для оценки критериев](./evaluateLLMs/criterion_methods.md)
4. [Итерации и анализ ошибок](./evaluateLLMs/error_analysis.md)
5. [Выбор модели для AI-систем](./evaluateLLMs/llm_choice.md)

# Prompt Engineering (18.03 - 01.04)
1. [Проектирование базового промпта](./promptEngineering/basicPrompt.md)
2. [Защита от атак](./promptEngineering/security.md)

# RAG (23.03 - 06.04)
1. [Введение в RAG](./rag/intro.md)
2. [Оценка качества ретривера](./rag/retriever_quality.md)

# Agents (30.03 - 13.04)
1. [Выбор Инструментов](./agents/tools.md)
2. [Оценка качества агента](./agents/eval.md)

# Fine Tuning (13.04 - 27.04)
1. [Fine Tuning](./fineTuning/fineTuning.md)

# Data Engineering (20.04 - 04.05)
1. [Подготовка данных](./data/data.md)

# Оптимизация инференса (27.04 - 11.05)
2. [Оптимизация инференса](./inference/inference.md)

